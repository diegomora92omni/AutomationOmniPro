...
describe('Test cases for the account creation flow', () => {
    ...
    it('Verify that an unregistered user can create an account', () => {
        ...
        cy.get('#firstname').type(datosUsuario.nombre[0])
        cy.get('#lastname').type(datosUsuario.apellido[0])
        cy.get('button.ui-datepicker-trigger.v-middle').click().should('be.visible')
        cy.get('.ui-datepicker-month').should('be.visible').select(datosUsuario.fechaNacimiento[0])
        cy.get('.ui-datepicker-year').should('be.visible').select(datosUsuario.fechaNacimiento[1])
        cy.get('.ui-state-default:eq(1)').click()
        cy.get('#gender').select(datosUsuario.genero[0])
        cy.get('#telefono').type(datosUsuario.telefono)
        cy.get('#rut').type(datosUsuario.rut[0])
        cy.get('#email_address').type(datosUsuario.email[0])
        cy.get('#password').type(datosUsuario.password[0])
        cy.get('#password-confirmation').type(datosUsuario.password[0])
    ...
    it('Verify that a registered user cannot create an account', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
    ...
    it('Verify account creation without subscribing to the newsletter', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
            ...
            cy.get('.ui-datepicker-year').should('be.visible').select(datosUsuario.fechaNacimiento[1])
            cy.get('.ui-state-default:eq(1)').click()
            cy.get('#gender').select(datosUsuario.genero[2])
            cy.get('#telefono').type(datosUsuario.telefono)
            cy.get('#rut').type(datosUsuario.rut[1])
            cy.get('#email_address').type(datosUsuario.email[1])
            cy.get('#password').type(datosUsuario.password[0])
            cy.get('#password-confirmation').type(datosUsuario.password[0])
            //Hacer clic en botón "crear una cuenta" para confirmar el registro
            cy.get('#send2').click()
    ...
    it('Verify that an account cannot be created when a mandatory field is not filled in', () => {
        // Hacer clic en el botón "Iniciar sesión"
        cy.get('.panel > .header > .link > a').click({ force: true })
        // Hacer clic en el botón de "Crear una cuenta"
        cy.get('.login-container .block-new-customer .action.primary').click()
        ...
        cy.get('#gender').select(datosUsuario.genero[0])
        cy.get('#telefono').type(datosUsuario.telefono)
        cy.get('#rut').type(datosUsuario.rut[2])
        //cy.get('#email_address').type(datosUsuario.email[0]) NO DILIGENCIAR CAMPO
        cy.get('#password').type(datosUsuario.password[0])
        cy.get('#password-confirmation').type(datosUsuario.password[0])
        //Hacer clic en botón "crear una cuenta" para confirmar el registro
        cy.get('#send2').click()
    ...
    // Test case #5: Verificar que no se pueda crear cuenta cuando el RUT no sea valido
    it('Verify that it is not possible to create an account when the RUT is not valid', () => {
        // Hacer clic en el botón "Iniciar sesión"
        cy.get('.panel > .header > .link > a').click({ force: true })
        // Hacer clic en el botón de "Crear una cuenta"
        cy.get('.login-container .block-new-customer .action.primary').click()
        ...
        cy.get('#gender').select(datosUsuario.genero[0])
        cy.get('#telefono').type(datosUsuario.telefono)
        cy.get('#rut').type('123') ///RUT Inválido
        cy.get('#email_address').type(datosUsuario.email[2])
        cy.get('#password').type(datosUsuario.password[0])
        cy.get('#password-confirmation').type(datosUsuario.password[0])
        //Hacer clic en botón "crear una cuenta" para confirmar el registro
        cy.get('#send2').click()
...
it('Verify that the password is hidden by default', () => {
    // Hacer clic en el botón "Iniciar sesión"
    cy.get('.panel > .header > .link > a').click({ force: true })
    // Hacer clic en el botón de "Crear una cuenta"
    cy.get('.login-container .block-new-customer .action.primary').click()
    // Rellenar el formulario de registro con datos de prueba
    cy.get('#firstname').type(datosUsuario.nombre[1])
    cy.get('#lastname').type(datosUsuario.apellido[1])
    cy.get('button.ui-datepicker-trigger.v-middle').click().should('be.visible')
    cy.get('.ui-datepicker-month').should('be.visible').select(datosUsuario.fechaNacimiento[0])
    cy.get('.ui-datepicker-year').should('be.visible').select(datosUsuario.fechaNacimiento[1])
    cy.get('.ui-state-default:eq(1)').click()
    cy.get('#gender').select(datosUsuario.genero[1])
    cy.get('#telefono').type(datosUsuario.telefono)
    cy.get('#rut').type(datosUsuario.rut[2])
    cy.get('#email_address').type(datosUsuario.email[2])
    cy.get('#password').type(datosUsuario.password[0]).should('have.attr', 'type', 'password')  
    cy.get('#password-confirmation').type(datosUsuario.password[0]).should('have.attr', 'type', 'password')  
})
    // Test case #7: Verificar que la contraseña sea visible cuando el usuario lo solicite
    it('Verify that the password is visible at the user´s request', () => {
        // Hacer clic en el botón "Iniciar sesión"
        cy.get('.panel > .header > .link > a').click({ force: true })
        // Hacer clic en el botón de "Crear una cuenta"
        cy.get('.login-container .block-new-customer .action.primary').click()
        ...
        it('Verify error message when the password does not match', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
            ...
            cy.get('#email_address').type(datosUsuario.email[2])
            cy.get('#password').type(datosUsuario.password[0])
            cy.get('#password-confirmation').type(datosUsuario.password[1])
            //Hacer clic en botón "crear una cuenta" para confirmar el registro
            cy.get('#send2').click()
            //Verificar que aparezca mensaje de error
        ...
        it('Verify that it is not possible to create an account with a registered email address', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
        ...
        it('Verify that an error message is displayed when the password does not meet the minimum length', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
            ...
            cy.get('#rut').type(datosUsuario.rut[2])
            cy.get('#email_address').type(datosUsuario.email[2])
            cy.get('#password').type(datosUsuario.password[2])
        ...
        it('Verify that an error message is displayed when the password does not comply with the different character classes', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
            ...
            cy.get('#rut').type(datosUsuario.rut[2])
            cy.get('#email_address').type(datosUsuario.email[2])
            cy.get('#password').type(datosUsuario.password[3])
        ...
        it('Verify account creation without selecting: Remember ¿Me What is this?', () => {
            // Hacer clic en el botón "Iniciar sesión"
            cy.get('.panel > .header > .link > a').click({ force: true })
            // Hacer clic en el botón de "Crear una cuenta"
            cy.get('.login-container .block-new-customer .action.primary').click()
            ...
            cy.get('#gender').select(datosUsuario.genero[0])
            cy.get('#telefono').type(datosUsuario.telefono)
            cy.get('#rut').type(datosUsuario.rut[3])
            cy.get('#email_address').type(datosUsuario.email[3])
            cy.get('#password').type(datosUsuario.password[0])
            cy.get('#password-confirmation').type(datosUsuario.password[0])
